...
namespace RavenIron.ValkyriesCargo.Core
{
    ...
    public sealed class EngineProbes
    {
        ...
        public EngineProbes()
        {
            ...
            Declare(Merchant, 6, "Character.m_name / m_faction / Faction.Players / IsOnGround, Humanoid.UnequipAllItems, MonsterAI.SetFollowTarget / GetFollowTarget / m_alertRange, BaseAI.Follow / MoveTo (the chain the walk-up rides) / SetPatrolPoint / m_aggravatable / m_passiveAggresive / m_randomMoveRange, NpcTalk, Player.GetClosestPlayer, ZNetScene.FindInstance / GetPrefab, ZDO.GetZDOID(hashPair), Chat.SetNpcText, Odin.m_despawn, EffectList.Create, Player.Interact(GameObject, bool, bool) / m_lastHoverInteractTime (Patch_Player_Interact's target and its stamp, both private)",
                                 "the merchant cannot be set up, pinned, spoken through or sent away (CargoMerchant); or the use key on him falls back to the first Interactable in component order (Patch_Player_Interact)");
