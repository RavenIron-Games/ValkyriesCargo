...
- **Wu'barrk** (Thorium Wu'barrk) - co-founder, Raven Iron. Design; Ingvar himself (`models/ingvar.fbx`: model, rig and
  animation), the Cargo Terminal, and the interface work under it:
  `Libs/SharedUI/GiltFrameTheme.cs` and `Libs/SharedUI/UIFocus.cs` are his VikingOS 0.9.8 shared
  source, MIT, vendored here and never edited.
