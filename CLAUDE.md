...
- **On 1.0 the three lists want the DISPLAY id: `V_<steamid>`** (read 2026-09-10 on Storm10). `ZNet.ListContainsId`
  still tries the bare number and `Steam_<id>`, then `PlatformUserID.FilterPlatformUserID` (Splatform.dll, a 1.0
  assembly the decompile tools do not yet cover) rewrites the platform to a one-letter display prefix — Steam `V`,
