...
| 🕊️ **A real approach** | The Valkyrie's flight is *planned* — a genuine line through the sky that every player on the map watches her fly. She comes in low enough that you can see what she is carrying. |
...
| 📬 **Nothing is lost** | A deal settled but not delivered — you crashed, you dropped — is redelivered when you come back. |
...
| `BackpackOnIngvar` | `on` | Whether he wears the pack when Backpacks is present |
| `BackpackOffset` / `Rotation` / `Scale` | — | Where the pack sits on his back |

---

## 🛠️ Compatibility

Built against **Valheim 0.221.12**. At boot the mod checks the game it is actually running on,
prints what it found, and any feature depending on something that has **moved** switches *itself*
off and says so — rather than taking your session down with it.

- ✅ Dedicated servers, listen hosts and single-player
- ✅ Run alongside a **117-plugin** modpack in testing
- ✅ Plays well with the family — Cairn, Undertow, FireFront, Ragnarok's Wrath, RavenEye and
  Yggdrasil's Reckoning
...
with real players: the roll, the flight, the drop, the walk-up, the terminal, deals across the wire,
the price curve, the persistence and the departure have all been watched on a screen. Some corners
are proven only in the log, and a few only off-game.
