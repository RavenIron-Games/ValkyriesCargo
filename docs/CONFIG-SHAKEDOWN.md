...
| `BackpackShelfMultiplier` | `2` | 1–4 | SERVER | `FillMarketRules` → `Shelf.Scaled(ShelfSize, this, BackpackMod.Present)` → `MarketRules.ShelfSize` | `Shelf.Scaled` clamps 1–`Shelf.MaxBackpackMultiplier` 4 and the product to 200 — **agrees** | true | NEW 2026-09-08, the backpack add-on; does nothing unless the named mod is loaded on the server |
