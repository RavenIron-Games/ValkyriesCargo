...
**The four version numbers are identical to Nomad's Windows baseline**: 0.221.12, net 36, player 43,
...
`ZNet.RPC_PeerInfo` — same members, and the unified diffs under each one are the same hunks Nomad's
...
Nomad's Windows sweep: **2** files only in the server (`PlayFabAuthWithCustomID`, `<PrivateImplementationDetails>`).
...
is present under this run's `baseline-client` tree where it was absent from Nomad's. `<PrivateImplementationDetails>`
...
not on anything this mod reaches. It is named nowhere in `ValkyriesCargo/`, same as Nomad's report already
says of it. The type-file-differs count is 35 here against Nomad's 40 for the same reason in the other
...
No `-beta public-test` shadow, same as Nomad's baseline: **checked live from this box, 2026-09-07**,
...
exactly the same six branches Nomad found on Windows — `public` (buildid `21981590`, matching his
