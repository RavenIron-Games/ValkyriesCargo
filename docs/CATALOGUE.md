...
- **Four meads** (`MeadHealthMajor`, `MeadHealthLingering`, `MeadStaminaLingering`, `MeadEitrMinor`): a brew is six
  meads off ten honey and forage, and through 0.1.4 a mead row was bought back UNCLAMPED whenever it was off the shelf
  (the Fair Market Act clamped only a shelf row). The 0.1.5 fix branch proposes clamping every row while the shelf
  rotates, which matters only for a row below its target (PROPOSED, docs/DECISIONS-WUBARRK.md §2); the eight proposed could absorb the whole 1,500 purse in one visit. Four
